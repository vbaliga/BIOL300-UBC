...
In this lab you will be given a bag with 25 cowrie shells. Think of
...
2.  Your TA will provide your group with a ziplock bag that has 25
    shells. Sample five shells from this bag. Put them in spots 1
    through 5 on the paper. This is your sample.
...
4.  Create a new spreadsheet in Excel or Google Sheets, and on the first
    ...
    five shells. Save the file as `Lab03_First_sample.csv` in your
    `DataForLabs` folder.

5.  In your R script for today's lab, import `Lab03_First_sample.csv`
...
6.  Next we will calculate the mean of the population of shells in your
    bag. Take each remaining shell one by one, measure its length, and
    record it on the sheet beside the next available number. Set the
...
7.  Create another blank spreadsheet in Excel or Google Sheets. Add the
    ...
    `Lab03_Full_population_shells.csv`.
...
A relatively easy way to do this is to store the random numbers in a
named vector, and then used that named vector to index from the full
population data:

```{r}
# Take a random sample of 5 integers and store it as "samp"
samp <- sample.int(n = 25, size = 5)
# These are the row numbers of population_data$length we'll use:
samp
```

If you then run `population_data$length[samp]`, you'll get the lengths
of these 5 specific shells only.
...
18. From your TA, get the list of the means from the first samples of
    ...
    data into a `data.frame` in R, naming the object `groups_means`.
