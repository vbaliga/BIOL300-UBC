...
![](formula_summaries/R_console.png){width="480"}
...
answer, 4, and it will label that answer with `[1]` to indicate that it
is the first element in the answer. (This is sort of annoying when the
...
![](formula_summaries/blank_R_script.png){width="480"}
...
In general, a name should start with a letter, but that can be followed
by letters or numbers. There can’t be any spaces or dashes, but
underscores work.
...
"camelCase" (capital letters = humps).

It may be tempting to try something like `weights-before-hospital`, but
this will not work as R will interpret this as subtraction, e.g.,
`weights` - `before` - `hospital`. Do **not** use this convention (often
called "Kebab-case", dashes = skewers).
...
1](Lab%201%20student%20data%20sheet.pdf), record the requested
information about yourself. This is optional; if you have any reason to
not want to record this (relatively innocuous) data about yourself, you
do not have to. If you feel that you would like to skip just one of the
bits of information and fill in the rest, that is fine too. The data
sheets do not identify students by name. Pass the sheet to the TA when
you are finished. We’ll use these data in subsequent labs.

<br> 2. Each week in BIOL 300, you will create an R script that captures
the code that you use to answer the questions, along with the answers.
Use a `#` at the beginning of each comment line.

a.  Open a new R script file. Start by adding comments with your name
    and the lab number (Lab 1) at the top.
...
b.  For each of the questions below, write the question number as a
    ...
    the answers as comments. Save the R script on your computer; you
    will upload the R script you write each week as part of the Canvas
    quiz.

For example, here is what the beginning of your script might look like.
...
a.  Open this script in RStudio.

b.  Run most or all of the commands in R. Did you get the same answers
    as shown in the text?

<br> 4. For each of the variables below, come up with a object name that
would be appropriate to use in R. Remember that camelCase and snake_case
are usually good conventions. If an object name is already given, edit
the name so that it will work appropriately in R.

a.  How much aspirin is given per dose for a patient

b.  Height (including neck and extended legs) of giraffes

c.  Object name: `temperature-(Celsius)`

d.  Object name: `televisions-per-person`
