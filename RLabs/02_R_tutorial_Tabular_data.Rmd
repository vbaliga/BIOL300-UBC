...
- Understand how data frames and tibbles organize information
...
These expansions are usually released in “packages”. An R package is a
collection of functions that can be used in addition to the built-in R
...
![](formula_summaries/Install_tidyverse.png){width="480"}

Installing an R package only needs to be done once on a given computer.
If installed correctly, the package will permanently available from that
...
tidyverse. This might feel initially confusing, but `tidyverse` itself
is a collection of separate packages. Therefore, `tidyverse` is
sometimes referred to as a "metapackage" (don't worry, that bit of
trivia won't be on an exam).

Before using the functions in these packages, we need to load
`tidyverse`. We do this with the `library()` function:

```{r message=TRUE, warning=TRUE, results='markup'}
library(tidyverse)
```

If the `tidyverse` was installed on your computer correctly, the
computer will show a brief status message like the one above and then
will be ready to go. In the first part of the message, you can see the
names of each individual R package within `tidyverse`, such as `dplyr`,
`forcats`, and so on. The numbers after each package name indicate the
version of each package that is on your computer (this can occasionally
be important if specific functions change across versions of a package).

Our labs will use functions from several packages within tidyverse,
including `ggplot2` and `dplyr`, and the above status message shows you
that they have been loaded up as part of doing `library(tidyverse)`.

If the package was not installed correctly (or at all), it will give you
an error message in red asking you to install the package.
...
Looking at the Environment tab in RStudio can help confirm that the data
were imported. The Environment allows you to see all objects that are in
R's memory and available for you to use.

![](formula_summaries/Environment.png){width="480"}

<br>

### Finding files in other places

*Note that for all labs that you turn in for BIOL 300, we expect you to
use `ABDLabs.Rproj` to start your session, and put any new data into the
`DataForLabs` folder. Do not use either of the examples in this section
in your R scripts for BIOL 300.*

When you work on a new data set outside of BIOL 300 labs, you will want
to store the data somewhere else. To import data into R from another
location on your computer, you need to know the full file path for the
file. For example, the file path to the titanic file on my Mac is
"/Users/vikram/Desktop/ABDLabs/DataForLabs/titanic.csv". On a Windows
computer, it would look something like
...
the file in a specific place, which may be outside your current working
directory:

```{r eval=FALSE}
titanicData <-
  read.csv("/Users/vikram/Desktop/titanic.csv",
    stringsAsFactors = TRUE
  )
```

Another alternative is to have R use your Finder or Windows Explorer to
help you select the right file. This can be occasionally useful if you
are importing a file for one-time usage and aren't worried so much about
the reproducibility of your code. *Do not use this in your R scripts for
BIOL 300.* Reading a file from any location can be done with
`file.choose()` like this:
...
This will bring up a new window where you navigate to find the file of
interest yourself. Note that the specific name of the file
(`titanic.csv`) isn't specified in the block of code; it's incumbent on
you to select the correct file.

*Again note that for all labs that you turn in for BIOL 300, we expect
you to use `ABDLabs.Rproj` to start your session, and put data into the
`DataForLabs` folder. Do not use either of the examples above in your R
scripts.* All data import should follow something like:
`object_name <- read.csv("DataForLabs/data.csv", stringsAsFactors = TRUE)`

This enforces a standard environment for all submitted work, which makes
life easier for you & your TAs.
...
characteristics of the data, the commands below can be useful.
...
![](formula_summaries/VIew_function_titanicData.png){width="680"}

Again, note that the `titanicData` object has 1313 rows and 7 columns. A
convention in R is to describe data frames according to the number of
rows first and then columns second. For example, `titanicData` would be
a 1313 x 7 object.

The horizontal and vertical scroll bars will allow you to see more of
the data compared to what may initially appear in the window. <br>
...
Sometimes, we'll deal with datasets that are quite "wide" (lots of
measured variables). These data may not lend themselves well to being
inspected via `View()` and/or you may just want to see basic information
about the names of variables.

`names()`, if used on a `data.frame` or `tibble`, will report the names
of each column:
...
The function `read.csv()` imports data into R as a `data.frame`. To work
with the data, it will be important to save it as an object in your R
Environment. For example, in the previous section we read in a data set
and saved it as `titanicData`. This object contains information about
passengers on the Titanic. It has seven variables, so it has seven
columns: `passenger_class`, `name`, `age`, `embarked`,
`home_destination`, `sex`, and `survive`.
...
"Indexing" means selecting specific parts of objects. In [Lab
1](01_R_tutorial_Intro_to_R.html#Vectors), we used square brackets `[ ]`
to look at specific elements within a vector.

Data frames and tibbles are often described by how many rows and columns
they contain. The convention is to list rows first: for example,
`titanicData` would be a 1313 x 7 object.

We can use this convention as a coordinate system for indexing specific
subsets of the data. To do this, we again use square brackets right next
to the object name and specify [row(s), column(s)] of interest. Remember
that in R, the indexing starts at 1, not 0 (*this is different from the
way Python works!*).

For example, to see the entry for the 6th row and 3rd column of
`titanicData`:
...
If you do not include a value in either the "row" or "column" spot, R
will include all rows or all columns. The comma within the brackets
`[ , ]` will still be necessary for R to interpret what you intend. The
following shows all columns for the 6th row:

```{r}
titanicData[6, ]
```

Indexing data can get a bit more complicated. Other operators such as
`:` or functions such as `c()` can be used to grab specific subsets of
data. This won't be a point of emphasis in BIOL 300; we're more likely
to subset our data based on factors using the `filter()` function
...
For example, to add the natural log of age as a column in the
...
you have loaded `tidyverse` into R using `library()`. The line
`libary(tidyverse)` only needs to be run once per session, and then you
can use any of the functions in the tidyverse packages, including
`filter()`.
...
`titanicData`, but it will only include the 463 rows for which `sex` was
`female`. Note the differences between `summary(titanicDataFemalesOnly)`
and what we saw when we ran `summary(titanicData)`.

Note that the syntax within `filter()` requires a double equal sign:
`==`. In R (and many other computer languages), the double equal sign
creates a statement that can be evaluated as `TRUE` or `FALSE`, whereas
a single equal sign may change the value of the object to the value on
the right-hand side of the equal sign. Here we are asking, for each
...
already. After it has installed, use `library()` to load it into your R
environment. A good practice is to write `library(tidyverse)` as one of
the first lines of code in each R script you write for BIOL 300.

<br> 2. In Lab 1, Question 1, you and your fellow students may have
recorded requested information about yourselves. You will now use
the data from all of these sheets to create a `.csv` file in
Excel or Google Sheets. Include all of seven of the variables for each of
the individuals in the data sheets. Be sure to use variable names that
are 1) legal in R and 2) intelligible to another person who may
encounter the file (remember that `snake_case` and `camelCase` are
preferred conventions). If any data are missing (e.g., the person chose to
not record a value), leave the corresponding cell blank. Save the data file 
that you make in the `DataForLabs` folder on your computer with the file name
`Lab01_section_measurements.csv`.
...
b.  The line of code below will not work because it contains mistakes.
    Make corrections to the code to import the csv into R as a data
    frame named `Lab01_section_measurements`. Explain each correction you
    made (there should be at least 2). Why is each one necessary?

    `Lab01_section_measurements <- read.csv(DataForLabs/Lab01_section_measurements.csv, strings = "TRUE")`
...
c.  Using the output of `summary()`, how many countries are from Asia in
    this data set?
...
<br> 4. A researcher wants to examine data from Japan only and writes
the line: `japan_data <- filter(countries, country = Japan)`. Would this
line work? If not, make corrections to the code. Then determine the value of
`life_expectancy_at_birth_female` for Japan, as found in `japan_data`.

<br> 5. Using the `countries` data, create a new data frame called
`AfricaData` that only includes data for countries in Africa.

a.  How many countries are included in `AfricaData`?

b.  What is the sum of the `total_population_in_thousands_2015` in
    `AfricaData`?
