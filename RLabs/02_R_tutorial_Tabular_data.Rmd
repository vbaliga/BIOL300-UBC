...
sometimes referred to as a "metapackage" (don't worry, that bit of trivia won't be on an exam).

Before using the functions in these packages, we need to load `tidyverse`.
...
already. After it has installed, use `library()` to load it
into your R environment. A good practice is to write `library(tidyverse)` as
one of the first lines of code in each R script you write for BIOL 300.
...
`AfricaData` that only includes data for countries in Africa via the
`filter()` function.
