...
use `ABDLabs.Rproj` to start your session, and put data into the
`DataForLabs` folder. Do not use either of the examples below in this
section in your R scripts for BIOL 300.*
...
`DataForLabs` folder. Do not use either of the examples above in this
section in your R scripts.* All data import should follow something
like:
...
`str()` will report the names of each column, along with a little more
descriptive information about the type of variable it contains. It is
somewhat of a hybrid of `summary()` and `names()`. "str" stand for
"structure".
...
"Indexing" means selecting specific parts of objects. In lab 1, we used
square brackets `[ ]` to look at specific elements within a vector.
