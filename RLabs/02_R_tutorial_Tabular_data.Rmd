...
similar data from previous students, which can be found in [this
PDF](DataSheetsLab4.pdf). Use the data sheets to create a `.csv` file in
Excel or Google Sheets. Include all of five of the variables for each of
...
preferred conventions). Save the data file that you make in the
`DataForLabs` folder on your computer with the file name
`Lab1_section_measurements.csv`.
...
b.  The line below will not work because it contains a couple errors.
    Make corrections to the code to import the csv into R as a data
    frame named `Lab1_section_measurements`. Explain each correction you
    made (there should be at least 2). Why is each one necessary?

`Lab1_section_measurements <- read.csv(DataForLabs/Lab1_section_measurements.csv, strings = "TRUE")`
...
line work? If not, make edits to the code. Then determine the value of
