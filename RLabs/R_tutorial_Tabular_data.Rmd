...
# [REPLACED IMAGE]{style="color:#ff5e00;"}
...
To see if the data were imported appropriately, you might want to run
the command
...
By supplying the full address, you are telling `read_csv()` to look for
the file outside your current working directory. Reading a file from any
location can be done with `read_csv()` like this:
...
The function `read_csv()` loads data into R as a `tibble`. A `tibble` is
a modern version of a `data.frame` that is commonly used with tidyverse
tools. In this course, most of the things we do with data frames also
work with tibbles. The data set is usually given a name, which lets us
tell R which data set to use. For example, in the previous section we
read in a data set and called it `titanicData`. This object contains
information about passengers on the Titanic. It has seven variables, so
it has seven columns: `passenger_class`, `name`, `age`, `embarked`,
...
![](formula_summaries/VIew_function_titanicData.png){width="600"}
...
the data than what may initially appear in the window.
...
# [Add mutate() and factor() here too?]{style="color:#ff5e00;"}
...
<br> 1. In Lab 1, Question 7, you and your fellow students may have
...
a.  Import this data file into R using `read_csv()`, and give the
    resulting data frame a name that makes sense to you.

b.  Use the `View()` function to check that you have read the data
    correctly.

c.  [Make a quick plot of VARIABLE against VARIABLE USING plot(X,Y)]{style="color:#ff5e00;"}

<br> 2. The data file called `countries.csv` in the `DataForLabs` folder
contains information about all the countries on Earth. Each row is a
country, and each column contains a variable.

a.  Use `read_csv()` to read the data from this file into a data frame
    called countries.
...
c.  [Using the output of `summary()`, how many countries are from Africa
    in this data set?]{style="color:#ff5e00;"} May not work without factor
...
<br> 3. Using the `countries` data again, create a new data frame called
`AfricaData`, that only includes data for countries in Africa. What is
the sum of the `total_population_in_thousands_2015` for this new data
frame?
