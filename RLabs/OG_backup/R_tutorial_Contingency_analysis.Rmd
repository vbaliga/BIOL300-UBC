---
title: "<em>Labs using R</em>: <br>7: Contingency analysis"
author: 
...
```
*This lab is part of a series designed to accompany a course using *The Analysis of Biological Data*. The rest of the labs can be found [here](index.html). This lab is based on topics in Chapter 9 of ABD.*
...
Add **$estimate** after the results of the **fisher.test()** function call to get the odds ratio estimate. For 
example, if we want to know the odds ratio for survival as a function of sex for the Titanic data, we write:
```{r}
...
This output shows that the $\chi$^2^ value for this test is 327.7, with 1 degree of freedom and a *P*-value less than 2.2 x 10^-16^. So we can reject the null hypothesis of no association between sex and survival on the Titanic.
...
[http://www.zoology.ubc.ca/~whitlock/Kingfisher/ContingencyAnalysis.htm](http://www.zoology.ubc.ca/~whitlock/Kingfisher/ContingencyAnalysis.htm)
...
After this paragraph, we reprint the last paragraph of Darwin's *Origin of Species*. You can [download a page with only this one paragraph](OriginsParagraph.pdf) to print out. (Print a paper version if you are reading this electronically.) Please read through this paragraph, and circle every letter "t".  Please proceed at a normal reading speed. If you ever realize that you missed a "t" in a previous word, do not retrace your steps to encircle the "t". You are not expected to get every "t", so don't slow down your reading to get the "t"s.
...
b. Use **factor()** to fix the order of "ill"  so that the favorable outcome comes first in the levels list. Do the same for each of the food variables that you will analyze, so that "Y" for people that eat the food preceded the "N" for people that didn't.

c.	Using **table()** and **chisq.test()$expected**, calculate the expected values for a $\chi$^2^ contingency test of the relationship between fruit salad and illness. Would it be legitimate to use a $\chi$^2^ contingency analysis to test this association? Why or why not? What test would be best to use?

d.	You want to know which food is most strongly associated with illness. For the sake of simplicity, let’s imagine that we have ruled out all the other foods except for spinach, baked ham, vanilla ice cream and chocolate ice cream. Use **fisher.test()** to calculate an odds ratio for the illness for each these foods. Which is the most likely vehicle of the disease?

e.	Using the food you think is the likely vehicle, what is the 95% confidence interval of the odds ratio for illness?

f.	For the food you decided in part *c* is the most likely vehicle, draw a mosaic plot to illustrate how many people got sick as a function of whether they ate this food.
...
<br>
2.  Return to the data you collected on your class during the first week. (If you are doing these outside of a class setting, anonymized data based on a previous classes answers is available as "studentSampleData.csv".) Is there an association between dominant hand and dominant foot?

<br>
3.  Human names are often of obscure origin, but many have fairly obvious sources. For example, "Johnson" means "son of John," "Smith" refers to an occupation, and "Whitlock" means "white-haired" (from "white locks"). In Lancashire, U.K., a fair number of people are named "Shufflebottom," a name whose origins remain obscure. 
...
a.	What is the odds ratio for the association between name and mode of movement? Give a 95% confidence interval for this odds ratio.

b.	Based on the confidence interval for the odds ratio, would you expect that a hypothesis test would find a significant association between name and movement type? Why or why not?

c.	Is there a significant difference between the name groups for mode of movement?

<br>
4. Falls are extremely dangerous for the elderly; in fact many deaths are associated with such falls. Some preventative measures are possible, and it would be very useful to have ways to predict which people are at greatest risks for falls.
...
a.	Draw a mosaic plot of the relationship between stopping to talk and falls.

b.	Carry out an appropriate hypothesis test of the relationship between "stops walking while talking" and falls.

c.	What is the odds ratio of this relationship? Give a 95% confidence interval. Which group has the higher odds of falling?
